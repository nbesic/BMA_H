...
H_all["SMA"] = np.nanmean(I[:,0:5], axis = 1)
...
H_mat=np.zeros((np.shape(z_new)[0],86))
...
H_mat_mean = np.nansum(H_mat, axis=0) / (SER_mat_nb - np.sum(np.isnan(H_mat) == 1, axis=0))
...
ser_mask["Average_H"] = H_mat_mean[ser1]
...
ax2 = plt.subplot2grid((1, 15), (0, 2), colspan = 2)
...
ax3 = plt.subplot2grid((1, 15), (0, 4), colspan = 2)
...
ax4 = plt.subplot2grid((1, 15), (0, 6), colspan = 2)
...
ax5 = plt.subplot2grid((1, 15), (0, 8), colspan = 2)
...
ax6 = plt.subplot2grid((1, 15), (0, 10), colspan = 3)
...
ax7 = plt.subplot2grid((1, 15), (0, 13), colspan = 2)

cmap7 = plt.cm.Greys

ser_mask.plot(ax = ax7, column = 'Average_H', cmap = cmap7)

norm = colors.Normalize(vmin = 0, vmax = ser_mask['Average_H'].max())
cbar = plt.cm.ScalarMappable(norm = norm, cmap = cmap7)
ax_cbar = fig.colorbar(cbar, ax = ax7, shrink = 0.85, ticks=[0., ser_mask['Average_H'].max()], location = "top")
ax_cbar.ax.set_xticklabels(['$0$', '$H_{\mathrm{SER}}^{\max}$'])
ax_cbar.ax.tick_params(labelsize = fs-2, top = True, labeltop = True)
...
ax7.set_title("$H_{\mathrm{SER}}$ ($H_{\mathrm{SER}}^{\max}$ = %.2f)" % ser_mask['Average_H'].max(), fontsize = fs-2)

fig.text(0.01, 0.5, str_mat, va='center', ha='center', rotation='vertical', fontsize = fs+2)
...
## Illustration - BMA vs. SMA (Fig. 7 in the manuscript)
...
ax1.set_title(str_mat+": Within variance (BMA)", fontsize = fs-1)
...
ax2.set_title(str_mat+": Between variance (BMA)", fontsize = fs-1)
...
ax3.set_title(str_mat+": Within $-$ between (BMA)", fontsize = fs-1)
...
ax4.set_title(str_mat+": Within variance (SMA)", fontsize = fs-1)
...
ax5.set_title(str_mat+": Between variance (SMA)", fontsize = fs-1)
...
ax6.set_title(str_mat+": Within $-$ between (SMA)", fontsize = fs-1)
...
ax1.set_xlabel("$M_1$",fontsize = fs-2)
ax1.set_ylabel("$NFI - $"+str_mat,fontsize = fs)

ax1.set_title("$M_1$: $R^2=$%.2f, $MBE=$%.2f,\n $NRMSE=$%.2f %%" % 
			  (r2_score(H_all_1["NFI"], H_all_1["Lang"]), np.mean(H_all_1["Lang"] - H_all_1["NFI"]),
	  		   np.sqrt(mean_squared_error(H_all_1["NFI"], H_all_1["Lang"]))/np.mean(H_all_1["NFI"])*100),fontsize = fs-2)
...
ax2.set_xlabel("$M_2$",fontsize = fs-2)
ax2.set_title("$M_2$: $R^2=$%.2f, $MBE=$%.2f,\n $NRMSE=$%.2f %%" % 
			  (r2_score(H_all_1["NFI"], H_all_1["Liu"]), np.mean(H_all_1["Liu"] - H_all_1["NFI"]),
	  		   np.sqrt(mean_squared_error(H_all_1["NFI"], H_all_1["Liu"]))/np.mean(H_all_1["NFI"])*100),fontsize = fs-2)
...
ax3.set_xlabel("$M_3$",fontsize = fs-2)
ax3.set_title("$M_3$: $R^2=$%.2f, $MBE=$%.2f,\n $NRMSE=$%.2f %%" % 
			  (r2_score(H_all_1["NFI"], H_all_1["Morin"]), np.mean(H_all_1["Morin"] - H_all_1["NFI"]),
	  		   np.sqrt(mean_squared_error(H_all_1["NFI"], H_all_1["Morin"]))/np.mean(H_all_1["NFI"])*100),fontsize = fs-2)
...
ax4.set_xlabel("$M_4$",fontsize = fs-2)
ax4.set_title("$M_4$: $R^2=$%.2f, $MBE=$%.2f,\n $NRMSE=$%.2f %%" % 
			  (r2_score(H_all_1["NFI"], H_all_1["Potapov"]), np.mean(H_all_1["Potapov"] - H_all_1["NFI"]),
	  		   np.sqrt(mean_squared_error(H_all_1["NFI"], H_all_1["Potapov"]))/np.mean(H_all_1["NFI"])*100),fontsize = fs-2)
...
ax5.set_xlabel("$M_5$",fontsize = fs-2)
ax5.set_title("$M_5$: $R^2=$%.2f, $MBE=$%.2f,\n $NRMSE=$%.2f %%" % 
			  (r2_score(H_all_1["NFI"], H_all_1["Schwartz"]), np.mean(H_all_1["Schwartz"] - H_all_1["NFI"]),
	  		   np.sqrt(mean_squared_error(H_all_1["NFI"], H_all_1["Schwartz"]))/np.mean(H_all_1["NFI"])*100),fontsize = fs-2)
...
ax6.set_xlabel("$BMA$",fontsize = fs-2)
ax6.set_title("$BMA$: $R^2=$%.2f, $MBE=$%.2f,\n $NRMSE=$%.2f %%" % 
			  (r2_score(H_all_1["NFI"], H_all_1["BMA"]), np.mean(H_all_1["BMA"] - H_all_1["NFI"]),
	  		   np.sqrt(mean_squared_error(H_all_1["NFI"], H_all_1["BMA"]))/np.mean(H_all_1["NFI"])*100),fontsize = fs-2)
...
ax7 = plt.subplot2grid((2, 7), (0, 6))

im7 = sns.kdeplot(data = H_all, x = "SMA", y = "NFI", color = "k", legend = False)

ax7.plot([0, np.nanmax(H_all["NFI"])], [0, np.nanmax(H_all["NFI"])], color = "k", linestyle = 'dashed', linewidth = 1)

H_all_1 = H_all.copy()
H_all_1 = H_all_1.dropna(subset=['NFI', 'SMA'])

ax7.set_xlabel("$SMA$",fontsize = fs-2)
ax7.set_title("$SMA$: $R^2=$%.2f, $MBE=$%.2f,\n $NRMSE=$%.2f %%" % 
			  (r2_score(H_all_1["NFI"], H_all_1["SMA"]), np.mean(H_all_1["SMA"] - H_all_1["NFI"]),
	  		   np.sqrt(mean_squared_error(H_all_1["NFI"], H_all_1["SMA"]))/np.mean(H_all_1["NFI"])*100),fontsize = fs-2)

plt.xticks(fontsize = fs-2)
plt.yticks(fontsize = fs-2)

ax7.set_xlim(0,np.nanmax(H_all["NFI"]))
ax7.set_ylim(0,np.nanmax(H_all["NFI"]))
...
## Illustration - influence of categorical variables on the spread (Fig. B1 in the manuscript)
...
plt.savefig("GMD_illustrations/Boxplots_"+str+"_"+str_dist+".png", dpi=my_dpi)
...
plt.savefig("GMD_illustrations/MNT_by_SER.png", dpi=my_dpi)
...
res3 = stats.linregress(MNT_elev_mat_mean[~np.isnan(MNT_elev_mat_mean)], Var_inter_mat_mean[~np.isnan(Var_inter_mat_mean)])

ax3.axline(xy1=(0, res3.intercept), slope = res3.slope)
...
res4 = stats.linregress(MNT_pente_mat_mean[~np.isnan(MNT_pente_mat_mean)], Var_inter_mat_mean[~np.isnan(Var_inter_mat_mean)])

ax4.axline(xy1=(0, res4.intercept), slope = res4.slope)
