...
while (delta > epsilon) & (iter < maxiter):
	...
	if pdf_ind == 1:
		...
		z_new = np.apply_along_axis(py_sweep, 1, bernoulli.pmf(np.tile(I[:, K][np.newaxis].T, [1, K]), I[:, range(K)]), w)
	...
	if pdf_ind == 1 or pdf_ind == 2:
		...
		delta = np.max([np.max(np.abs(z - z_new)), np.max(np.abs(w - w_new))])
		print(delta)
...
if pdf_ind == 1 or pdf_ind == 2:
	...
	var_inter = np.nansum(np.tile(w_new, [N, 1]) * (I[:, range(K)] - np.nansum(np.tile(w_new, [N, 1]) * I[:, range(K)], axis = 1))**2, axis = 1)
	
	var_inter_1 = np.nansum(np.tile(w_1, [N, 1]) * (I[:, range(K)] - np.nansum(np.tile(w_1, [N, 1]) * I[:, range(K)], axis = 1))**2, axis = 1)

	# Within-model variance

	var_intra = np.nansum(np.tile(w_new, [N, 1]) * I[:, range(K)] * (1 - I[:, range(K)]), axis = 1)
	
	var_intra_1 = np.nansum(np.tile(w_1, [N, 1]) * I[:, range(K)] * (1 - I[:, range(K)]), axis = 1)
